...
router.get("/parents-going/:eventID", rejectUnauthenticated, (req, res) => {
  const queryText = `
  SELECT DISTINCT "user"."name", "user"."id", "booking"."going_dates", "booking"."invite-link" AS "booking_invite_link"
    FROM "user"
    ...
    SELECT  "user"."name", "user"."id", NULL AS "going_dates", NULL AS "booking_invite_link"
    FROM "user"
